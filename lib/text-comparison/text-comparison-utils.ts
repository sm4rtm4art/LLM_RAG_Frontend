...
export async function compareTexts(text1: string, text2: string): Promise<ComparisonResult> {
...
export async function extractImportantParts(text1: string, text2: string): Promise<string> {
  // Simulate API call or heavy computation
  ...
  const addedLines = differences.filter((part) => part.added).map((part) => part.value)
  ...
  const removedLines = differences.filter((part) => part.removed).map((part) => part.value)
