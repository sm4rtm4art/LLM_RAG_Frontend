...
import { compareTexts, extractImportantParts } from "@/lib/text-comparison/text-comparison-utils"
import type { ComparisonResult, TextFile, ViewMode } from "@/types/text-comparison"
    ...
    <div className="space-y-4">
      <Card className="bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700">
        <CardHeader>
          ...
          <CardDescription>{t("textComparison.uploadInstructions")}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div>
              <h3 className="text-sm font-medium mb-2">{t("textComparison.file1")}</h3>
              <FileUploader
            ...
            <div>
              <h3 className="text-sm font-medium mb-2">{t("textComparison.file2")}</h3>
              <FileUploader
          ...
          <div className="flex flex-wrap gap-2 justify-between items-center">
            ...
            {comparisonResult && <ComparisonViewSelector viewMode={viewMode} onViewModeChange={setViewMode} />}
          </div>
          ...
          {isComparing && (
            <div className="text-center py-8">
              <p className="text-muted-foreground">{t("textComparison.loadingComparison")}</p>
            </div>
          )}

          {isExtracting && (
            <div className="text-center py-8">
              <p className="text-muted-foreground">{t("textComparison.processingFiles")}</p>
            </div>
          )}

          {comparisonResult && !extractionResult && (
            <div className="mt-6">
              <TextComparisonView file1={file1} file2={file2} comparisonResult={comparisonResult} viewMode={viewMode} />
            </div>
          )}
