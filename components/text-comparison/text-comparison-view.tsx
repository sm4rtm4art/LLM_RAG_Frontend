...
export function TextComparisonView({ file1, file2, comparisonResult, viewMode }: TextComparisonViewProps) {
  const { t } = useI18n()
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <ScrollArea className="h-[500px] p-4">
            <pre className="text-xs whitespace-pre-wrap font-mono">{file1.content}</pre>
          </ScrollArea>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent className="p-0">
          <ScrollArea className="h-[500px] p-4">
            <pre className="text-xs whitespace-pre-wrap font-mono">{file2.content}</pre>
          </ScrollArea>
        </CardContent>
      </Card>
    </div>
        ...
        <ScrollArea className="h-[500px] p-4">
          <div className="text-xs font-mono whitespace-pre-wrap">
            {comparisonResult.diffLines.map((line, index) => {
              if (line.added) {
                return (
                  <div key={index} className="bg-green-100 dark:bg-green-900/30">
                    {line.value}
                  </div>
                )
              }
