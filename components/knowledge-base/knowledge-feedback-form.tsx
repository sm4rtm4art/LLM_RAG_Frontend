...
export function KnowledgeFeedbackForm({ articleId, onSubmit, onCancel }: KnowledgeFeedbackFormProps) {
  const { t } = useI18n()
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-medium">{t("knowledgeBase.feedback.title")}</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Textarea value={comment} onChange={(e) => setComment(e.target.value)} rows={4} className="resize-none" />
        </div>
